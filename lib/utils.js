...
(function (window) {
    ...
    // Javascript class type
        class2type = {},
    // Array methods
        core_slice = utils.emptyArray.slice,
        core_concat = utils.emptyArray.concat,
        core_push = utils.emptyArray.push,
    // Object methods
    ...
    var core_indexOf = function (list, elem) {
        var i = 0,
            len = list.length;
        for (; i < len; i++) {
            if (list[i] === elem) {
    ...
    utils.chain = function (obj, callbacks) {
        var isArr = utils.isArrayLike(obj),
            cbs = utils.toArray(callbacks);
        ...
        for (; i < length; i++) {
            ...
            if ((options = arguments[i]) != null) {
                ...
                for (name in options) {
                    ...
                    if (deep && copy && (utils.isPlainObject(copy) || (copyIsArray = utils
                        .isArray(copy)))) {
                        if (copyIsArray) {
                            copyIsArray = false;
